...
extern int xilinx_perfmon_run_begin(void);
extern int xilinx_perfmon_run_end(void);
 ...
 */
#define PATTERN_SRC		0x90
#define PATTERN_DST		0x00
#define PATTERN_COPY		0x30
#define PATTERN_OVERWRITE	0x10
...
int stop_perf = 0;
static int dmaperf_func(void)
{
	while(!kthread_should_stop() && !stop_perf)
	{
		xilinx_perfmon_run_begin();
		xilinx_perfmon_run_end();
		//cpu_relax();
	}
...
static int dmatest_slave_func(void *data)
{
	...
	/* JZ: limit testing scope here */
	iterations = 1;
	test_buf_size = 1920*1080*4;//4096*25;

	dma_addr_t phy_dma[2];
	u8* virt_dma[2];
	...
	virt_dma[0] = dma_alloc_coherent(tx_chan->device->dev, test_buf_size*bd_cnt, &phy_dma[0], GFP_KERNEL);
	virt_dma[1] = dma_alloc_coherent(rx_chan->device->dev, test_buf_size*bd_cnt, &phy_dma[1], GFP_KERNEL);

	thread->srcs = kcalloc(src_cnt+1, sizeof(u8 *), GFP_KERNEL);
	if (!thread->srcs)
		goto err_srcs;
	for (i = 0; i < src_cnt; i++) {
		thread->srcs[i] = virt_dma[0] + (test_buf_size*i);
	...
	if (!thread->dsts)
		goto err_dsts;
	for (i = 0; i < dst_cnt; i++) {
		thread->dsts[i] = virt_dma[1] + (test_buf_size*i);
		//kmalloc(test_buf_size, GFP_KERNEL);
		if (!thread->dsts[i])
			goto err_dstbuf;
	}
	...
	while (!kthread_should_stop()
		...
		src_off = 0;//(src_off >> align) << align;
		dst_off = 0;//(dst_off >> align) << align;
		...
		rx_tmo = msecs_to_jiffies(10000);
		tx_tmo = msecs_to_jiffies(10000);
		...
		trace_printk("%s: dma_async_issue_pending\n", __func__);

		if (z==200)
		{
			struct task_struct *task = kthread_run(dmaperf_func, NULL, "perf:%s-%s",
		...
		pr_debug("%s: dma_async_is_tx_complete\n", __func__);

		if (tx_tmo == 0) {
			pr_warning("%s: #%u: tx (%u) test timed out\n",
				   thread_name, total_tests - 1, z);
			failed_tests++;
			break;
		} else if (status != DMA_SUCCESS) {
		...
		pr_debug("%s: dma_async_is_rx_complete\n", __func__);
	...
	tx_chan->device->device_control(tx_chan, DMA_TERMINATE_ALL, 0);
	rx_chan->device->device_control(rx_chan, DMA_TERMINATE_ALL, 0);
		}

		stop_perf = 1;
		pr_info("%s: done\n", __func__);
		//pr_debug("%s: dma_unmap_single\n", __func__);
		/* Unmap by myself (see DMA_COMPL_SKIP_DEST_UNMAP above) */
		for (i = 0; i < dst_cnt; i++)
		...
		trace_printk("%s: verifying dest buffer...\n",
				thread->task->comm);
		error_count += dmatest_verify(thread->dsts, 0, dst_off,
				0, PATTERN_DST, false);
		error_count += dmatest_verify(thread->dsts, dst_off,
		...
		error_count += dmatest_verify(thread->dsts, dst_off + len,
				test_buf_size, dst_off + len,
				PATTERN_DST, false);
		trace_printk("%s: verifying source buffer...\n", thread_name);
		error_count += dmatest_verify(thread->srcs, 0, src_off,
				0, PATTERN_SRC, true);
		error_count += dmatest_verify(thread->srcs, src_off,
				src_off + len, src_off,
	...
	ret = 0;
	//for (i = 0; thread->dsts[i]; i++)
		dma_free_coherent(tx_chan->device->dev, test_buf_size * bd_cnt, virt_dma[0], phy_dma[0]);
		//kfree(thread->dsts[i]);
err_dstbuf:
	kfree(thread->dsts);
err_dsts:
	//for (i = 0; thread->srcs[i]; i++)
		dma_free_coherent(rx_chan->device->dev, test_buf_size * bd_cnt, virt_dma[1], phy_dma[1]);
		//kfree(thread->srcs[i]);
err_srcbuf:
	kfree(thread->srcs);
err_srcs:
